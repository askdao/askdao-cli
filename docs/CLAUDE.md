...
- **kol-quickstart-windows.md** — 面向 KOL 本人的 Windows 上手指南（v0.4 2026-06-11，onboarding 计划 S6 产物；§1 含 PowerShell `.\` 前缀与用户级 PATH 写法——首位 KOL 真机反馈）：安装 exe（平台直发 或 §1 `gh release download` 自助下载）+ PATH → askdao.ai 注册 + 订阅模式激活（M4 硬前提）→ `auth login`（**内置 askdao-mcp 自动配置**，不再需要平台单独交付 token；准备清单已删 MCP token 项）→ 验证连接（claude `/mcp` / codex 提问）+ `mcp setup`/`--print` 备用 → 本地调试（链 askdao-mcp-reference.md）→ `agent edit`/`deploy` → 常见问题 + Windows 路径速查
- **askdao-mcp-reference.md** — askdao-mcp 工具参考（v1.0 2026-06-11，面向 Skill 编写者）：6 能力域 19 工具全清单（listenhub 8 / elevenlabs 4 / google+openai 文生图 2 / market 2 / sec 3，定义从 gateway 源码 tools/*/index.ts 提取）+ 各域关键参数/典型链路 + **Skill 编写注意事项 9 条**（产物 URL 时效 图10min/音频60min 立即消费、异步两形态、先查资源不硬编码 voice/speaker、配额预查、错误是文本非异常、凭证零出现、SKILL.md description 触发语义、数据边界免责、组合范式）。quickstart §4/§5 链入
- **windows-kol-onboarding-plan.md** — 第一个 Windows 外部 KOL 接入计划（v0.1 2026-06-10）：现状盘点（代码跨平台 ready / 分发+token发放+文档 missing）+ 短期方案 S1-S7（outside collaborator 单仓 Read、交叉编译直发 exe、`GATEWAY_TOKENS_JSON` 手工 entry、`claude mcp add` 一行配置、quickstart 文档、Windows 真机验证门槛）+ 中期方案 M1-M5（GoReleaser 发布工程、`askdao mcp setup` + conductor `/api/v1/cli/mcp-credentials` 两步走 token 动态化、Codex MCP TOML、onboarding 产品化、开源决策点）+ 优先级时间线 + 风险清单
- **WxArticle-Project-To-Claude-Plugin.md** — 外部参考文章存档（公众号「鬼马风」2026-06-07）：guige-skills 从 install.sh/symlink 迁移到 Claude Code Plugin 的实录（三套 manifest fan-out + CI 校验 + SessionStart hook + description 触发原则）。§9.11 Plugin 调研的野生验证案例，增量输入已落档 design.md §9.11「野生案例验证」小节
