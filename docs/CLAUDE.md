...
- **kol-quickstart-windows.md** — 面向 KOL 本人的 Windows 上手指南（v0.1 2026-06-10，onboarding 计划 S6 产物）：安装 exe + PATH → askdao.ai 注册补 KOL 资料 → `auth login` → askdao-mcp 配置（token 走用户环境变量 `ASKDAO_MCP_TOKEN`，Claude Code `claude mcp add` user scope / Codex `config.toml` `bearer_token_env_var` / Codex 项目需项目根 `.mcp.json` env 展开 workaround，因 cli 暂不扫 TOML）→ 本地调试 → `agent edit`/`deploy` → 常见问题 + Windows 路径速查。gateway 单一 endpoint `https://mcp.askdao.ai/mcp` 已对源码核实（README 旧 `/<name>/mcp` 分路径写法已废弃）
