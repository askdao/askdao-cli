...
- **kol-quickstart-windows.md** — 面向 KOL 本人的 Windows 上手指南（v0.2 2026-06-11，onboarding 计划 S6 产物）：安装 exe + PATH → askdao.ai 注册 + 订阅模式激活（M4 硬前提）→ `auth login`（**内置 askdao-mcp 自动配置**，不再需要平台单独交付 token；准备清单已删 MCP token 项）→ 验证连接（claude `/mcp` / codex 提问）+ `mcp setup`/`--print` 备用 → 本地调试（链 askdao-mcp-reference.md）→ `agent edit`/`deploy` → 常见问题 + Windows 路径速查
