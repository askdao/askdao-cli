...
- **kol-quickstart-windows.md** — 面向 KOL 本人的 Windows 上手指南（v0.5 2026-06-11，onboarding 计划 S6 产物）：§1 一行命令安装 `irm https://askdao.ai/install.ps1 | iex`（仓库开源 + install/ 脚本上线后；手动下载 + PATH 流程降级为备用方案；升级用 `askdao update`）→ askdao.ai 注册 + 订阅模式激活（M4 硬前提）→ `auth login`（**内置 askdao-mcp 自动配置**）→ 验证连接（claude `/mcp` / codex 提问）+ `mcp setup`/`--print` 备用 → 本地调试（链 askdao-mcp-reference.md）→ `agent edit`/`deploy` → 常见问题 + Windows 路径速查
...
- **windows-kol-onboarding-plan.md** — 第一个 Windows 外部 KOL 接入计划（v0.2 2026-06-11）：现状盘点 + 短期方案 S1-S7（**S1 outside collaborator 已作废**——仓库 2026-06-11 转 public）+ 中期方案 M1-M5 全线落地记录（M1 GoReleaser ✅ / M2 mcp setup ✅ / M3 Codex TOML ✅ / M4 核心 ✅ / **M5 开源 ✅ 2026-06-11**：gitleaks 零泄露 + flip public + `install/` 一键脚本 + `askdao update` 自升级）+ 优先级时间线 + 风险清单（私有 Release gh 摩擦已解除）
