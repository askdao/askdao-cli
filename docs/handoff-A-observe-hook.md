...
> 配套（按需读）：`docs/HANDOFF.md` v0.8 段、`docs/review-observe-pivot-2026-05-21.md`、`~/.claude/plans/anthropic-managed-pure-pixel.md` 的 M4c。

---

## 0. 项目背景（全新窗口必读）

**askdao-cli 是什么**：AskDAO 体系内唯一开源的子项目（信任锚点）。Go 1.26 单二进制 CLI，KOL 在本地项目目录跑一行命令 → 扫描技术栈 → 生成 Anthropic Managed Agents 配置 → 经 Conductor 部署。设计哲学：本地隐私（扫描不上传文件内容）、确定性优先（L1-L3 用工业库，LLM 只做最后推荐）、review-and-edit（审阅推荐草稿而非空白模板）。
...
**当前代码状态**：
- 仓库 `/Users/sunmu/WorkSpace/askdao-cloud/askdao-cli`，分支 `main`。第一步（v0.8 `agent edit` 本地 Web 工作台）**已 merge**（#32 #33）。
...
- **skill 激活就是一次工具调用**。`Skill` 是 Claude Code 一等内置工具，skill 名明文落在 `tool_input.skill`。实证：`{"name":"Skill","input":{"skill":"spelling-homework-generator"}}`（正是 skill_pipeline 真实样例）。`PreToolUse(matcher:"Skill")` 直接拿到。
...
- **开工前唯一要补的一次实测**（spike R3）：带子代理（Subagent）的 session，确认子代理内的 skill/MCP 调用是否冒泡到主 `PreToolUse`。其余已被双重证据锁定。**先和哥确认这个再大改。**
...
- 起步：① 读 spike 报告全文 ② 跟哥确认 R3 子代理冒泡实测 ③ 实现 `/api/observe` → `--observe` → 预勾。
...
```
哥，我做 askdao-cli 的 observe hook 预勾这条线（v0.8 第二步 A）。
