...
- **main.go** — subcommand router。无第三方 CLI 框架；stdlib `flag` + 手写 dispatch。一级命令 `auth / agent / version / help`；`agent` 二级 dispatch 到 `edit / deploy`；`auth` 二级 dispatch 到 `login / status / logout`。`version` 是 var（非 const）：release 构建由 GoReleaser 经 `-ldflags -X main.version` 注入真实版本，源码字面量 `0.0.0-dev` 只出现在 from-source 构建。
