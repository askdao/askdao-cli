...
- **edit.go** — `askdao agent edit [--dir path] [--harness id] [--no-ui] [--force]`。**v0.8 核心命令**：`runEdit` → `loadOrScan`（存在 `askdao-agent.yml` 则加载 + 重扫拿 skill/MCP 候选；否则跑 `pipeline.Run` 生成草稿 + `writeBaseline` 落 `.askdao/`）→ 默认主题色（`webstudio.DefaultThemeForCategory`）→ 启 `webstudio.Serve`（`OnSave`=`writeAgentSpec` 写 yaml；`OnDeploy`=写 yaml + `deployFromDir` + `studioDeployError` 把 kol_profile 错转「去 askdao.ai 补填」引导 + `deployResultLine`）。`--no-ui` 降级为扫描+写草稿退出（CI/headless）。复用同包：`chooseLLMClient` / `readSpec` / `resolveServerAndToken` / `deployFromDir` / `ensureAskdaoDir` / `defaultAgentName`。
