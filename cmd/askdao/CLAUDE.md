...
- **edit.go** — `askdao agent edit [--dir path] [--harness id] [--no-ui] [--force] [--observe]`。**v0.8 核心命令**：`runEdit` → `loadOrScan`（存在 `askdao-agent.yml` 则加载 + 重扫拿 skill/MCP 候选；否则跑 `pipeline.Run` 生成草稿 + `writeBaseline` 落 `.askdao/`）→ 默认主题色（`webstudio.DefaultThemeForCategory`）→ 启 `webstudio.Serve`（`OnSave`=`writeAgentSpec` 写 yaml；`OnDeploy`=写 yaml + `deployFromDir` + `studioDeployError` 把 kol_profile 错转「去 askdao.ai 补填」引导 + `deployResultLine`）。**`--observe`**：观测真实 claude session 预勾真正用到的 skill/MCP —— 进流程先 `observe.SweepStale` 清残留，`webstudio.Serve` 的 `OnReady(port)` 回调里 `observe.Install` 写临时 PreToolUse hook（指向 `/api/observe`）+ `printObserveGuide` 引导 KOL 另开终端跑 `claude`，`defer` cleanup 字节级还原（零残留三件套见 [../../internal/observe/CLAUDE.md](../../internal/observe/CLAUDE.md)）。`--no-ui` 降级为扫描+写草稿退出（CI/headless）。复用同包：`chooseLLMClient` / `readSpec` / `resolveServerAndToken` / `deployFromDir` / `ensureAskdaoDir` / `defaultAgentName`。
- **deploy.go** — `askdao agent deploy [--dir path] [--harness id] [--force] [--bio text]`。读 `<dir>/askdao-agent.yml` **原始字节**（不 re-marshal）+ 解析；`.askdao/recommendation.yml` 存在则先 `render.DiffAgentSpec` 显示 KOL 改动；每个 `custom_local` skill 经 `internal/deploy.ZipDir` 打 zip（harness 中性：`filepath.Base` 切上级路径）；经 `deploy.Client.Deploy` 以 `multipart/form-data` POST conductor `/api/v1/cli/deploy`。`409 kol_profile_required`（ADR-P21）→ `setupKolProfile`（CLI prompt bio + `SetupKol` + 重跑）；`ErrBlockingWarnings`（HIGH translation）→ `RenderTranslationWarnings(ViewAll)` + `--force` gating。**v0.7.1 update-mode**：`DeployResponse.Created` / `PreviousManagedVersion` 区分 `Created` vs `Updated (vN→vN+1)`。**v0.8 新增复用函数**：`packageSkills(dir, spec)`（枚举 + zip custom_local skill，含 `resolveSkillDir` —— project 相对路径 join dir，user scope 的绝对/`~` 路径直接解析，供工作台勾选的全局 skill 打包）+ `deployFromDir(ctx, dir, harness, force)`（读 yaml + packageSkills + Deploy，无交互，返回 typed error；被 `edit` 的一站式 OnDeploy 复用）。helper：`setupKolProfile` / `printDeployResult` / `printDeployProgress` / `formatSkillRef` / `toRenderWarnings` / `resolveServerAndToken`（env 成对 > credentials.json > error）/ `readSpec`。
- **\*\_test.go** — `cmd_test.go`：`edit --no-ui`（写 askdao-agent.yml + detection.json）/ `deploy 无 conductor URL 错出` / `deploy 含 diff 显示 before/after`；`deploy_test.go`（`httptest` 假 conductor）：e2e happy（验 multipart + skill zip）/ `409 kol_profile_required` → SetupKol → 重跑 / HIGH-warning gating ±`--force` / 缺 skill 目录 exit 1。`captureStdout` 临时 redirect helper。
...
- **KOL Profile 不在本地 CLI/工作台**：归 askdao.ai 云端 web（认证后未填则云端补）；`edit` 的 OnDeploy 遇 `kol_profile_required` 引导去 askdao.ai。`deploy`（CLI）暂仍保留 `--bio` 交互握手。
