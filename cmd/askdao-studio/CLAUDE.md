...
- **app.go** — `App`（Wails bound-method 宿主 + `ctx`）+ `NewApp` / `startup(ctx)` + `StudioOptions`（桌面注入 webstudio 的 `Data`/`OnSave`/`OnDeploy`）。**阶段2 骨架 + 阶段3 登录**：`placeholderSpec` 占位 StudioData + `Desktop=true`；`authState`/`startLogin`/`loginPoll`/`logout` + `openBrowser` 接 `internal/auth` device flow(复用 CLI 同一 `credentials.json`,桌面/CLI 登录互通)→ 注入 webstudio `OnAuthState`/`OnLogin`/`OnLoginPoll`/`OnLogout`。**阶段4-5 真实接线**（替换 stub）：`scan`（`runtime.OpenDirectoryDialog` 选文件夹 → `pipeline.Run` + `llmClient`（登录→`recommender.ConductorClient`/否则 `MockClient`）→ `BuildStudioData` 换 placeholder）/ `save`（`syncNetworking` + 写 `askdao-agent.yml`）/ `deploy`（读 yaml + **`deployflow.PackageSkills` 单源** + `auth.Load` + `deploy.Client.Deploy`，组装镜像 cmd/askdao 的 deployFromDir）/ `spec`（`OnSpec` 动态返回 `currentData`）；注入 webstudio `OnSpec`/`OnScan`/`OnSave`/`OnDeploy`。`StudioData.NeedsScan`（placeholder 时 true）驱动 studio.html 选文件夹覆盖层。**app.go 逻辑不 import wails（除 runtime.OpenDirectoryDialog），但同 package main 含 main.go，只 CI 编译**。
