...
- `DeploymentPayload` 是「上传清单」的真相源：`PayloadEntry.Path` 目录以 `/` 结尾、`Bytes`/`Files` 为递归总和。**v0.7 起所有 custom skill 一律进 Includes**（vendored 与 repo-原生 同等对待），origin 信息携带在 `PayloadEntry.Reason`（`"repo-native"` 或 `"vendored: <source> @ <hash>"`）。Conductor 端 deploy adapter 按这份清单打包上送，不再做"按 references 重装"。
