...
| `detected_mcp_configs` | `DetectMCPConfigs` | project `.mcp.json` + user scope(claude `~/.claude.json` / cowork app-data) + 远程(url/http/sse)→url 归一兼容标记 |
