...
- **harness_scope.go** — v0.8 harness 感知层。`ScanScopeOpts{HomeDir}` + `harnessConventions` 表（claude marker `.claude` → user `~/.claude/skills` + `~/.claude.json`；codex marker `.agent`、cursor `.cursor` 的 user 路径留 nil，TODO 待 Phase 2 核对）。`activeHarnesses(root)` 按工作目录 marker 决定扫哪些 harness 的 user 根目录（避免无关 harness 的全局 skill 噪音）；`harnessForProjectDir(rel)` 把 project 候选路径映射到 harness 标签。被 skills_dir.go / mcp_config.go 消费。
