...
- **selfupdate.go**: `Updater`（Repo/DownloadBase/Client/Out/ExePath/GOOS/GOARCH 全部可注入，零值落真实 GitHub 端点）。`Latest` 走 `releases/latest` 302 重定向解析版本（**不碰 api.github.com**，规避匿名 60/hr/IP 限流，与 `install/` 脚本同策）。`Run(ctx, currentVersion, force)`：取 latest 版本 → 同版本且非 force 返回 `ErrUpToDate` → 下载本平台 archive + checksums.txt SHA256 校验 → 提取二进制 → 原子换装（temp 同目录保证同文件系统；**Windows 运行中 exe 不能覆盖但能 rename**：旧 exe 先挪 `.old` 再上位，`.old` best-effort 删 + 下次 Run 开头清扫）
