...
L3 启发式层 —— 移植 railwayapp/nixpacks 的 Provider trait 抽象 + python.rs / node/mod.rs 的探测逻辑，外加 askdao-cli 自加的 dep→apt 反向映射表。issue #4 落 Python + Node 两个 P0 provider 与共享底座；Go / Rust provider 留 issue #5。
...
- **apt_map.go** — `InferAptPackages(pkgs)` 双表反向映射：`pythonAptMap`（移植 nixpacks python.rs:240-268 + askdao-cli 自加 Pillow / lxml / cryptography / pycurl / weasyprint）+ `npmAptMap`（puppeteer 12 个 apt deps、playwright 10 个、prisma / sharp / canvas）。仅 IsProd=true 的包参与；同 apt 包多 dep 触发时保留首条 reason。Phase 1 起码要有这两张表的 80%（spike 报告 §4）。
...
- **\*\_test.go** — `provider_test.go` 含 Provider 接口 conformance 断言（`var _ Provider = (*PythonProvider)(nil)` 编译期检查）+ App/Env 单测；`python_test.go` 覆盖 PM 优先级 7 case + env override + FastAPI / Django plan + 空项目；`node_test.go` 覆盖 PM 优先级 5 case + Next.js plan + puppeteer 12 apt deps + package.json fallback；`apt_map_test.go` 覆盖去重 / dev 跳过 / PEP 503 归一 / pip+npm 混合。
...
无新增三方依赖；用标准库 `regexp` / `encoding/json` + `internal/types`。

## 字段输出对应（detection.json）

| Detection 字段 | provider 函数 | 备注 |
|---|---|---|
| `detected_frameworks` | `Plan.Frameworks` | 多源证据加权 |
| `inferred_apt_packages` | `Plan.SystemPkgs["apt"]` | 来自 InferAptPackages 反向映射表 |
...
- issue #5 加 `go.go` / `rust.go`（同接口，不同 detect 信号 + apt 表项可能为空）
