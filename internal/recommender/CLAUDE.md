...
- **llm.go** — `LLMClient` 接口 + 两实现：
  - `ConductorClient`：HTTP POST `/api/v1/cli/recommend`，bearer 鉴权可选，默认 90s 超时（覆盖 LLM tail latency）。响应解析后会校验 `apiVersion == askdao.ai/v1`，避免 conductor 端意外升级 schema 静默通过。**另加 `FetchModelClasses(ctx, harness)`（GET `/api/v1/cli/model-classes` 旧 `classes[]` 三档）+ 模块级 `FetchModelClassesOrFallback(ctx, baseURL, token, harness)`：conductor 不可达/未登录时降级 `types.FallbackModelClasses()`（仅 slug/label/blurb、无 concrete id → 二进制不含模型 id，真 id 部署时由 conductor 从 model_class 解析）；cloud#84 再加 `FetchModelCatalog(ctx)`（同端点 `?harness=all` 的 `models[]` 白名单，两 harness 合集）+ `FetchModelCatalogOrEmpty(ctx, baseURL, token)`（离线/登出/旧 conductor 一律空切片，Studio 据此回退三档药丸；白名单刻意无本地兜底），两者共用 `fetchModelClasses`；全部 HTTP 往返收敛在泛型 `doJSON[T]`、三个模块级 graceful wrapper 收敛在 `fetchOr[T]`。另加 `FetchAppConfig`（GET `/api/v1/cli/config`）+ 模块级 `FetchStudioAssistantID(ctx, baseURL, token)`：读回桌面内嵌助手的 agent id（服务端常量），任何失败/未登录返 `""` 让侧栏降级静态帮助（同 FetchModelClassesOrFallback 优雅蓝本）。**
