...
- **diff.go** — `DiffAgentSpec(a, b)` 显式字段级 walker（不走 reflection）覆盖 KOL 实际编辑的字段（metadata / persona / capabilities / mcp_servers / workspace.packages / preferred_harness / #303 outcomes.{enabled,rubric,max_iterations} + schedule.{enabled,mode,cron,timezone,task,notify_channel}（mode 空展开为 personal），nil-safe 经 `outcomesView`/`scheduleView` 零值展开）。MCP 用名字做 union diff（add/remove/modify）。`RenderDiff` 输出 design.md §3.5 deploy preview 形态：`field.path:` / `-  before` / `+  after` 三行块。
- **summary.go** — `RenderSummary(r, SummaryInput)` 块顺序：PERSONA / SKILLS / MCP SERVERS / CAPABILITIES / RUNTIME / AUTOMATION（#303 outcomes+schedule 摘要行，schedule 行含 mode，两块皆缺时整节跳过——pre-#303 spec 渲染字节不变保 golden）/ SUBSCRIBER ONBOARDING / TRANSLATION WARNINGS。`SummaryInput` 装 AgentSpec 之外的额外材料（dev/prod 计数、被过滤的 MCP server 列表、translation warnings 等）。**v0.7 起 SummaryInput 删 `PersonaFileSize` / `PersonaFileNote`** —— persona 全在 `spec.Persona.SystemPrompt` literal block 内，单一真相源（design.md §9.15）。
- **payload.go** — `RenderPayload(r, root, DeploymentPayload, ProjectArchetype, full)` 渲染部署清单。`full=true`（`askdao bundle`）：archetype 行 + evidence + `WILL UPLOAD`（每条 include；skill 类条目附 inline origin tag `skill (repo-native)` 或 `skill (vendored: <source> @ <hash>)`；非 skill 目录附 immediate-children 摘要）+ `EXCLUDED`（每条 + reason）+ ignore sources 行。**v0.7 删 SKILL REFERENCES section** —— 所有 custom skill 一律上传，vendored 只是元数据标签。`full=false`（`askdao detect --summary` 末尾）：三行精简（archetype / N files X KB will upload (M skills: X native + Y vendored) · K excluded / "run `askdao bundle`"）。`humanSize` 渲字节、`childSummary` 走一层 ReadDir、`countSkillOrigins` 按 Reason 前缀分类 skill 条目。
