...
- **skills_test.go** — `TestResolveSkillDir`（四分支表驱动）+ `TestPackageSkills_FrontmatterValidation`（缺 name / 缺 description / name 撞名拒绝 + 完整 frontmatter 通过）。提取自 `cmd/askdao/skill_package_test.go`；`runDeploy` 的 e2e 回归（`TestDeploy_UserScopeAbsolutePath`）仍留 cmd/askdao。
...
- **单源反漂移**：CLI 与桌面共用 `PackageSkills`，避免各自复刻 `resolveSkillDir` 四分支（旧内联循环 `filepath.Join(dir, absPath)` 拼错全局 skill 绝对路径 → "directory not found" 的 bug 教训，v0.8.1 消除）。
- **提取动因**：Go 不允许跨 `package main` import，桌面（`cmd/askdao-studio`）复用 CLI 的 skill 打包逻辑必须落 `internal`；deploy 纪律又不收 types/scanner，故新开编排包。
