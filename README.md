...
| `askdao agent deploy [--dir path] [--spec file] [--harness id] [--force] [--confirm-downgrade]` | Package selected skills + push `askdao-agent.yml` to askdao.ai; `--spec` deploys another spec file from inside the same package (for a package that ships one spec per runtime line); `--harness` overrides `preferred_harness` for this one deploy without rewriting the yaml |
